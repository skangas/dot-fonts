...
{
print STDERR <<EOT;
usage:	bdftopsf [+h][+u|-u][+1|-1]|[-h[+r|-r]] [+o|-o OUTPUT]
	[--] [INPUT [TABLE...]]
...
INPUT	RAW and PSF1: a 256 or 512 character BDF file with
	...
	with width between 1 and 1200, height between 1 and
	2400 and number of characters between 1 and 2048.
...
TABLE	A duplicate unicodes table. Each line must either be
	...
	unicode data only, otherwise treated as error.

If no input or output are specified, the standard output is
the standard input and output are used.
...
{
print STDERR <<EOT;
bdftopsf.pl 0.2.0, Copyright (C) 2005 Dimitar Toshkov Zhekov
...
($width > 0 && $width <= 1200) || die("$0: $ARGV[0]: width $width zero or too big\n");
($height > 0 && $height <= 2400) || die("$0: $ARGV[0]: height $height zero or too big\n");
($chars > 0 && $chars <= 2048) || die("$0: $ARGV[0]: CHARS $chars zero or too big\n");
